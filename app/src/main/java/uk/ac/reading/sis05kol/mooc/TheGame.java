...
public class TheGame extends GameThread{
    ...
    private float mBallX = -500;
    private float mBallY = -500;
    ...
    //The point is the top left corner, not middle, of the ball
    //Initially at -100 to avoid them being drawn in 0,0 of the screen
    private float mSmileyBallX = -500;
    private float mSmileyBallY = -500;
    ...
    //Initially at -100 to avoid them being drawn in 0,0 of the screen
    private float[] mSadBallX = new float[5];
    ...
    @Override
    public void setupBeginning(int level) {
        ...
        mBallY = mCanvasHeight / 10;

        //Place Paddle in the middle of the screen
        mPaddleX = mCanvasWidth / 2;

        //Place SmileyBall in the middle of the screen
        mSmileyBallX = mCanvasWidth / 2;
        mSmileyBallY = 5 * mCanvasHeight / 11;

        //initialise sad faces off the screen
        for(int n=0; n<5; n++) {
            mSadBallX[n] = -500;
            mSadBallY[n] = -500;
        }

        //check level and load appropriate number of sad faces
        if(level == 1) {
            //Place one SadBall underneath the SmileyBall
            mSadBallX[0] = mCanvasWidth / 2;
            mSadBallY[0] = 9 * mCanvasHeight / 16;

        } else if(level == 2) {
            //Place two SadBalls forming a pyramid with the SmileyBall
            mSadBallX[0] = mCanvasWidth / 3;
            mSadBallY[0] = 5 * mCanvasHeight / 8;

            mSadBallX[1] = mCanvasWidth - mCanvasWidth / 3;
            mSadBallY[1] = 5 * mCanvasHeight / 8;
        }else if(level == 3){
            //Place three SadBalls forming a pyramid underneath the SmileyBall
            mSadBallX[0] = mCanvasWidth / 3;
            mSadBallY[0] = 5 * mCanvasHeight / 8 + 100;

            mSadBallX[1] = mCanvasWidth / 2;
            mSadBallY[1] = 9 * mCanvasHeight / 16 + 200;

            mSadBallX[2] = mCanvasWidth - mCanvasWidth / 3;
            mSadBallY[2] = 5 * mCanvasHeight / 8 + 100;
        }else if(level == 4){
            //Place four SadBalls in a row underneath the SmileyBall
            mSadBallX[0] = mCanvasWidth / 5;
            mSadBallY[0] = 5 * mCanvasHeight / 8;

            mSadBallX[1] = 2 * mCanvasWidth / 5;
            mSadBallY[1] = 5 * mCanvasHeight / 8;

            mSadBallX[2] = 3 * mCanvasWidth / 5;
            mSadBallY[2] = 5 * mCanvasHeight / 8;

            mSadBallX[3] = 4 * mCanvasWidth / 5;
            mSadBallY[3] = 5 * mCanvasHeight / 8;
        }else if(level == 5){
            //Place five SadBalls in a wiggly line underneath the SmileyBall
            mSadBallX[0] = mCanvasWidth / 6;
            mSadBallY[0] = 2 * mCanvasHeight / 3;

            mSadBallX[1] = 2 * mCanvasWidth / 6;
            mSadBallY[1] = 3 * mCanvasHeight / 4;

            mSadBallX[2] = 3 * mCanvasWidth / 6;
            mSadBallY[2] = 2 * mCanvasHeight / 3;

            mSadBallX[3] = 4 * mCanvasWidth / 6;
            mSadBallY[3] = 3 * mCanvasHeight / 4;

            mSadBallX[4] = 5 * mCanvasWidth / 6;
            mSadBallY[4] = 2 * mCanvasHeight / 3;
        }
        //level returns to 1 after level 5, determined in the updateGame method below
    ...
    protected void updateGame(float secondsElapsed) {
        ...
        if(mMinDistanceBetweenBallAndPaddle >= distanceBetweenBallAndPaddle) {
            ...
            mSmileyBallY = mSmileyBallY - 500;
            if(mSmileyBallY - mSmileyBall.getHeight()/2 <= 0 || score == 100){
        ...
        for(int i = 0; i < mSadBallX.length; i++) {
            ...
            if(mMinDistanceBetweenBallAndPaddle >= distanceBetweenBallAndPaddle) {
                ...
                //Move sad face down 5 pixels
                mSadBallY[i] = mSadBallY[i] + 20;
                if(mSadBallY[i] + mSadBall.getHeight()/2 >= mCanvasHeight){
